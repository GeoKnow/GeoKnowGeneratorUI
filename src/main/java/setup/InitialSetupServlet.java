 ...
 *
 * This servlet initializes RDF store for framework (creates users, system graphs, etc.).
 * If it is called with parameter reset=true, than setup data will be removed and created once again.
 * If it is called with parameter check==true, than it just check if the store is set up.
 */
public class InitialSetupServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        boolean reset = resetStr==null ? false : Boolean.parseBoolean(resetStr);

        //already set up
        if (setupManager.isSetUp() && !reset) {
            resp.getWriter().print("System is already set up. To reset system use parameter reset=true.");
            return;
        }
