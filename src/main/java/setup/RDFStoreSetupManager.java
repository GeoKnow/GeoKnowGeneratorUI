...
public class RDFStoreSetupManager {
    ...
    public void setUp(FrameworkConfiguration config, boolean reset) throws Exception {
        ...
        if (authSparqlUserExist)
            throw new Exception("Auth SPARQL user " + config.getAuthSparqlUser()
                    + " already exists");

        // check if system graphs doesn't exist
        RdfStoreManagerImpl publicRdfStoreManager = new RdfStoreManagerImpl(config
        ...
        userManager
                .setRdfGraphPermissions(config.getAuthSparqlUser(), config.getSettingsGraph(), 3);
        userManager
                .setRdfGraphPermissions(config.getAuthSparqlUser(), config.getAccountsGraph(), 3);
        ...
        queryString = "INSERT DATA { GRAPH <" + config.getInitialSettingsGraph() + "> { "
                + os.toString() + " } }";
        os.close();
        log.debug(queryString);
    ...
    public void clear(FrameworkConfiguration config) throws Exception {
        ...
        FrameworkUserManager frameworkUserManager = config.getFrameworkUserManager();
        RdfStoreManager rdfStoreManager = new SecureRdfStoreManagerImpl(config
                .getAuthSparqlEndpoint(), config.getAuthSparqlUser(), config
                .getAuthSparqlPassword());
