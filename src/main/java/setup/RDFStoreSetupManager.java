...
public class RDFStoreSetupManager {
    ...
    public void setUp(FrameworkConfiguration config, boolean reset)
	    throws Exception {
	...
	boolean authSparqlUserExist = userManager.checkUserExists(
		config.getAuthSparqlUser(), null);
	if (authSparqlUserExist)
	    throw new Exception("Auth SPARQL user "
		    + config.getAuthSparqlUser() + " already exists");

	// check if system graphs doesn't exist
	RdfStoreManagerImpl publicRdfStoreManager = new RdfStoreManagerImpl(
		config.getPublicSparqlEndpoint());
	...
	Iterator<JsonNode> bindingsIter = rootNode.path("results")
		.path("bindings").getElements();
	while (bindingsIter.hasNext()) {
	    JsonNode bindingNode = bindingsIter.next();
	    String graph = bindingNode.path("g").path("value").getTextValue();
	    if (graph.equals(config.getSettingsGraph())
		    || graph.equals(config.getAccountsGraph())
		    || graph.equals(config.getGroupsGraph())
		    || graph.equals(config.getInitialSettingsGraph())) {
	...
	// create user
	userManager.createUser(config.getAuthSparqlUser(),
		config.getAuthSparqlPassword());
	...
	SecureRdfStoreManagerImpl frameworkRdfStoreManager = new SecureRdfStoreManagerImpl(
		config.getAuthSparqlEndpoint(), config.getAuthSparqlUser(),
		config.getAuthSparqlPassword());
	...
	userManager.setRdfGraphPermissions(config.getAuthSparqlUser(),
		config.getSettingsGraph(), 3);
	userManager.setRdfGraphPermissions(config.getAuthSparqlUser(),
		config.getAccountsGraph(), 3);
	userManager.setRdfGraphPermissions(config.getAuthSparqlUser(),
		config.getGroupsGraph(), 3);
	userManager.setRdfGraphPermissions(config.getAuthSparqlUser(),
		config.getInitialSettingsGraph(), 3);
	...
	settingsModel.write(os, "N-TRIPLES");
	queryString = "INSERT DATA { GRAPH <" + config.getSettingsGraph()
		+ "> { " + os.toString() + " } }";
	os.close();
	frameworkRdfStoreManager.execute(queryString, null);
	...
	queryString = "INSERT DATA { GRAPH <" + config.getAccountsGraph()
		+ "> { " + os.toString() + " } }";
	os.close();
	frameworkRdfStoreManager.execute(queryString, null);

	// create users from framework configuration
	FrameworkUserManager frameworkUserManager = config
	...
	while (results.hasNext()) {
	    ...
	    String accountName = soln.get("accountName").asLiteral()
		    .getString();
	    String password = soln.get("password").asLiteral().getString();
	    String email = soln.get("mailto").toString()
    ...
    // clear rdf store using configuration: drop system graphs, sparql auth
    // user, all registered users (if exists)
    public void clear(FrameworkConfiguration config) throws Exception {

	VirtuosoUserManager virtuosoUserManager = config
		.getVirtuosoUserManager();

	log.info("Checking if system SPARQL user exists");
	if (!virtuosoUserManager.checkUserExists(config.getAuthSparqlUser(),
		null)) {
	...
	FrameworkUserManager frameworkUserManager = config
		.getFrameworkUserManager();
	RdfStoreManager rdfStoreManager = new SecureRdfStoreManagerImpl(
		config.getAuthSparqlEndpoint(), config.getAuthSparqlUser(),
		config.getAuthSparqlPassword());
