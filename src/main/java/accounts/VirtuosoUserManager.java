...
//        executeUpdate(getConnection(), "USER_SET_OPTION('" + name + "', 'DAV_ENABLE', 1)");
    }

    @Override
    public void dropUser(String name) throws Exception {
        executeUpdate(getConnection(), "USER_DROP('" + name + "')");
    }
    ...
    public void setDefaultRdfPermissions(String user, int permissions) throws Exception {
        executeUpdate(getConnection(), "DB.DBA.RDF_DEFAULT_USER_PERMS_SET ('" + user + "', " + permissions + ")");
    }

    @Override
    public void setRdfGraphPermissions(String user, String graph, int permissions) throws Exception {
        executeUpdate(getConnection(), "DB.DBA.RDF_GRAPH_USER_PERMS_SET ('" + graph + "', '" + user + "', " + permissions + ")");
    }
    ...
    public void setDefaultGraphPermissions(String graph, int permissions) throws Exception {
        executeUpdate(getConnection(), "DB.DBA.RDF_GRAPH_USER_PERMS_SET ('" + graph + "', 'nobody', " + permissions + ")");
    }
