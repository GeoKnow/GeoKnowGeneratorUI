...
public class RdfStoreManagerImpl implements RdfStoreManager {
    ...
    public String execute(String sparqlQuery, String responseFormat) throws Exception {
        System.out.println("Rdf store manager for " + endpoint + ": execute query " + sparqlQuery);
        String format = responseFormat==null ? URLEncoder.encode("application/sparql-results+json", encoding) : URLEncoder.encode(responseFormat, encoding);
        String urlParameters = "format=" + format + "&query=" + URLEncoder.encode(sparqlQuery, encoding);
