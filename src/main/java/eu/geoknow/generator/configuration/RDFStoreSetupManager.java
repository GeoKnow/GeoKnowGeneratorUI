...
public class RDFStoreSetupManager {
  ...
  private static final Logger log = Logger.getLogger(RDFStoreSetupManager.class);
  ...
  public RDFStoreSetupManager() throws IOException {
    // init
  ...
  public void setUp(boolean reset) throws Exception {
    ...
    rdfStoreUserManager.setRdfGraphPermissions(config.getWorkbenchSystemAdmin(),
        config.getInitialSettingsGraph(), UserManager.GraphPermissions.WRITE);
    rdfStoreUserManager.setRdfGraphPermissions(config.getWorkbenchSystemAdmin(),
        config.getJobsGraph(), UserManager.GraphPermissions.WRITE);
    rdfStoreUserManager.setRdfGraphPermissions(config.getWorkbenchSystemAdmin(),
        config.getAuthSessionsGraph(), UserManager.GraphPermissions.WRITE);

    // setup components graph
    setupComponentsGraph();

    Model settingsModel = ModelFactory.createDefaultModel();
    settingsModel.add(FrameworkConfiguration.getDatasetModel());
    settingsModel.add(FrameworkConfiguration.getComponentsModel());
    settingsModel.add(FrameworkConfiguration.getOntologyModel());
    ...
    QueryExecution qexec =
        QueryExecutionFactory.create(queryString, FrameworkConfiguration.getConfigurationModel());
    ...
    queryString =
        "INSERT DATA { GRAPH <" + config.getInitialSettingsGraph() + "> { " + os.toString()
            + " } }";
    os.close();
    log.debug(queryString);
    ...
    os = new ByteArrayOutputStream();
    FrameworkConfiguration.getOntologyAccountsModel().write(os, "N-TRIPLES");
    queryString =
    ...
    qexec = QueryExecutionFactory.create(query, FrameworkConfiguration.getConfigurationModel());
  ...
  public void clear() throws Exception {
    ...
    frameworkRdfStoreManager.dropGraph(config.getInitialSettingsGraph());
  ...
  private void checkGraphs() throws Exception {
    ...
    while (bindingsIter.hasNext()) {
      ...
      if (graph.equals(config.getComponentsGraph()) || graph.equals(config.getSettingsGraph())
          ...
          || graph.equals(config.getInitialSettingsGraph()) || graph.equals(config.getJobsGraph())
          || graph.equals(config.getAuthSessionsGraph())) {
  ...
  private void setupComponentsGraph() throws Exception {
    ...
    // TODO: how to handle that each time a user is created, he has to have
    // corresponding privileges to this graph?
