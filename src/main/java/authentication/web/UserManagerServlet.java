...
public class UserManagerServlet extends HttpServlet {
	...
	if (!correctToken)
	    throw new ServletException("Invalid token " + token + " for user "
		    + currentUser);
	...
	if ("getProfiles".equals(mode)) {
	    ...
	    try {
		userProfiles = frameworkUserManager
			.getAllUsersProfilesExtended();
	    } catch (Exception e) {
		throw new ServletException(e);
	    }
	    ...
	    String username = rootNode.path("profile").path("username")
		    .getTextValue();
	    String email = rootNode.path("profile").path("email")
		    .getTextValue();
		...
		Iterator<JsonNode> readableGraphsIter = readableGraphsNode
			.getElements();
		while (readableGraphsIter.hasNext())
		    readableGraphs
			    .add(readableGraphsIter.next().getTextValue());
	    }
		...
		Iterator<JsonNode> writableGraphsIter = writableGraphsNode
			.getElements();
		while (writableGraphsIter.hasNext())
		    writableGraphs
			    .add(writableGraphsIter.next().getTextValue());
	    }
	    String role = rootNode.path("profile").path("role") == null ? null
		    : rootNode.path("profile").path("role").getTextValue();
		...
		throw new ServletException("Failed to create account "
			+ username, e);
	    }
		...
		try {
		    ...
		    throw new ServletException("Failed to set role " + role
			    + " for user " + username, e);
		}
	    }
	    ...
	    String username = rootNode.path("profile").path("username")
		    .getTextValue();
	    Collection<String> readableGraphs = new ArrayList<String>();
	    Iterator<JsonNode> readableGraphsIter = rootNode.path(
		    "readableGraphs").getElements();
	    while (readableGraphsIter.hasNext())
		readableGraphs.add(readableGraphsIter.next().getTextValue());
	    Collection<String> writableGraphs = new ArrayList<String>();
	    Iterator<JsonNode> writableGraphsIter = rootNode.path(
		    "writableGraphs").getElements();
	    while (writableGraphsIter.hasNext())
    ...
    private void setGraphsAccess(String username,
	    Collection<String> readableGraphs, Collection<String> writableGraphs)
	    throws Exception {
	Collection<String> oldReadableGraphs = frameworkUserManager
		.getReadableGraphs(username);
	...
	Collection<String> oldWritableGraphs = frameworkUserManager
		.getWritableGraphs(username);
