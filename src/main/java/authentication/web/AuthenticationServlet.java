...
/**
 * Servlet provides some authentication functions: login, logout, register new
 * user, restore password, change password.
 * 
 * For some types of errors it sends error code and description in response. In
 * these cases response has json format: {code : ERROR_CODE, message :
 * ERROR_DESCRIPTION}.
 * 
 * Error codes: 1 - user already exists (during user registration, user with the
 * same name or e-mail already exists) 2 - incorrect old password (change
 * password) 3 - user doesn't exists (in restore password)
 */
